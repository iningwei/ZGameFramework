...
public class AES
{
    #region 加密(这个和文哥那边服务器的解密能对应，就这样吧)
    public static string Encrypt(string EncryptString, string EncryptKey)
    {
        return Convert.ToBase64String(Encrypt(Encoding.Default.GetBytes(EncryptString), EncryptKey));
    }
    public static byte[] Encrypt(byte[] inByte, string key)
    {
        if (inByte.Length == 0) { throw (new Exception("明文不得为空")); }
        ...
        byte[] outByte = encryptor.TransformFinalBlock(inByte, 0, inByte.Length);
