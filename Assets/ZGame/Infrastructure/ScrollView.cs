//使用hybridCLR热更框架，该脚本放到热更代码中会报错：MissingMethodException: AOT generic method not instantiated in aot.
//官方给出方案
//https://hybridclr.doc.code-philosophy.com/docs/help/commonerrors

//故这里考虑把该脚本放到非热更代码中，即：Infrastructure 程序集下
/*
 ...
 * Date    : 2018/08/11
 * Version : v1.0
 * Describe: 无限滚动列表，添加onItemRender事件后，设置numItems数量后自动渲染
 */
using System;
...
namespace ZGame.UGUIExtention
{
    ...
    public class ScrollView : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutGroup
    {
        ...
        public class ScrollLastRenderEvent : UnityEvent<int, Transform> { }

        public class ScrollRenderBackLastEvent : UnityEvent<int, int, Transform> { }
        ...
        private class ItemInfo
        {
            ...
            public void UpdateIndex(int virIdx, uint len)
            {
                ...
                //UnityEngine.Debug.Log($"隐藏：{lastRenderIndex} 展示：{actualIndex} 差值：{lastRenderIndex - actualIndex}");
        ...
        public ScrollRenderBackLastEvent onItemRenderBackLastIndex { get { return m_OnItemRenderBackLastIndex; } set { m_OnItemRenderBackLastIndex = value; } }
        [SerializeField]
        private ScrollLastRenderEvent m_OnItemLastRenderIndex = new ScrollLastRenderEvent();
        public ScrollLastRenderEvent onItemLastRenderIndex { get { return m_OnItemLastRenderIndex; } set { m_OnItemLastRenderIndex = value; } }
        ...
        public void SetMovePosition(Vector2 vPos)
        {
            if (!IsActive())
                return;
        ...
        protected void LateUpdate()
        {
            ...
            if (m_ViewBounds != m_PrevViewBounds || m_ContentBounds != m_PrevContentBounds || m_Content.anchoredPosition != m_PrevPosition)
            {
                ...
                //UnityEngine.Debug.Log(normalizedPosition.y);
        ...
        #region 下拉刷新
        float viewRefreshMinInterval;
        float refreshScrollDist;
        GameObject refreshTipObj;
        RectTransform refreshTipRect;
        Action onPageRefresh;

        bool refreshPageTrigger = false;
        float lastViewRefreshTime = 0;
        /// <summary>
        /// 设置页面的下拉刷新
        /// </summary>
        /// <param name="onPageRefresh">刷新回调</param>
        /// <param name="refreshTipTran">刷新提示的UI节点，这里会控制该节点的展示、隐藏以及显示位置</param>
        /// <param name="refreshScrollDist">触发下拉刷新的滑动幅度，默认120像素</param>
        /// <param name="viewRefreshMinInterval">避免频繁的刷新请求服务器，设置的最细时间间隔</param>
        public void SetViewRefresh(Action onPageRefresh, Transform refreshTipTran, float refreshScrollDist = 120, float viewRefreshMinInterval = 3f)
        {
            this.onPageRefresh = onPageRefresh;
            this.refreshTipObj = refreshTipTran.gameObject;
            this.refreshTipRect = refreshTipTran.GetComponent<RectTransform>();
            this.refreshScrollDist = refreshScrollDist;
            this.viewRefreshMinInterval = viewRefreshMinInterval;
            refreshTipTran.gameObject.SetActive(false);
            refreshTipTran.SetParent(viewport);
            refreshTipRect.anchorMin = new Vector2(0, 1);
            refreshTipRect.anchorMax = new Vector2(0, 1);
        }

        void RefreshView()
        {
            if (this.onPageRefresh == null) return;
            if (m_Dragging)
            {
                if (content.localPosition.y < -refreshScrollDist)
                {
                    refreshPageTrigger = true;
                    refreshTipObj.SetActive(true);
                }
                else
                {
                    refreshPageTrigger = false;
                    refreshTipObj.SetActive(false);
                }
                refreshTipRect.anchoredPosition = new Vector2(viewport.rect.width / 2, content.anchoredPosition.y + this.refreshScrollDist / 2);
            }
            else
            {
                if (refreshPageTrigger)
                {
                    refreshTipObj.SetActive(false);
                    refreshPageTrigger = false;
                    if (Time.time - lastViewRefreshTime > viewRefreshMinInterval)
                    {
                        lastViewRefreshTime = Time.time;
                        onPageRefresh?.Invoke();
                    }
                }
            }
        }
        ...
        private void UpdatePosition(bool forceRender = false)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                if (item.renderable)
                {
                    m_OnItemLastRenderIndex.Invoke(item.lastRenderIndex, item.transform);
                    m_OnItemRender.Invoke(item.actualIndex, item.transform);
                    m_OnItemRenderBackLastIndex.Invoke(item.lastRenderIndex, item.actualIndex, item.transform);
                }

                item.renderable = false;
                item.lastRenderIndex = item.actualIndex;
            }
        }
