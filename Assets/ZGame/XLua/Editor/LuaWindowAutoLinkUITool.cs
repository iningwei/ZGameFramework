...
public class LuaWindowAutoLinkUITool : Editor
{
    ...
    private static void genLuaItem(StringBuilder descSB, StringBuilder contentSB, Transform trans, string path)
    {
        ...
        else if (trans.name.EndsWith("Btn"))
        {
            ...
            else if (trans.name.EndsWith("Switch2Btn"))
            {
                descSB.AppendFormat("\t\t---@field public {0} ZGame.UGUIExtention.Switch2Button\r\n", trans.name);
                contentSB.AppendFormat("\t\tuis.{0}=ui:Find(\"{1}\"):GetComponent(typeof(CS.ZGame.UGUIExtention.Switch2Button))\r\n", trans.name, path);
            }
            else
            {
        ...
        {
            descSB.AppendFormat("\t\t---@field public {0} TMPro.TMP_Text\r\n", trans.name);
            contentSB.AppendFormat("\t\tuis.{0}=ui:Find(\"{1}\"):GetComponent(typeof(CS.TMPro.TMP_Text))\r\n", trans.name, path);
            //之前的Text组件已经全面弃用
            /*if (trans.name.EndsWith("TMPTxt"))
            {
            }
            else
            {
        ...
        else if (trans.name.EndsWith("InputField"))
        {
            if (trans.name.EndsWith("TMPInputField"))
            {
                descSB.AppendFormat("\t\t---@field public {0} TMPro.TMP_InputField\r\n", trans.name);
                contentSB.AppendFormat("\t\tuis.{0}=ui:Find(\"{1}\"):GetComponent(typeof(CS.TMPro.TMP_InputField))\r\n", trans.name, path);
            }
            else
            {
                descSB.AppendFormat("\t\t---@field public {0} UnityEngine.UI.InputField\r\n", trans.name);
                contentSB.AppendFormat("\t\tuis.{0}=ui:Find(\"{1}\"):GetComponent(typeof(CS.UnityEngine.UI.InputField))\r\n", trans.name, path);
            }
        }
        ...
        else if (trans.name.EndsWith("ScrollView"))
        {
            descSB.AppendFormat("\t\t---@field public {0} ZGame.UGUIExtention.ScrollView\r\n", trans.name);
            contentSB.AppendFormat("\t\tuis.{0}=ui:Find(\"{1}\"):GetComponent(typeof(CS.ZGame.UGUIExtention.ScrollView))\r\n", trans.name, path);
        }
        else if (trans.name.EndsWith("ScrollRect"))
        {
            descSB.AppendFormat("\t\t---@field public {0} UnityEngine.UI.ScrollRect\r\n", trans.name);
            contentSB.AppendFormat("\t\tuis.{0}=ui:Find(\"{1}\"):GetComponent(typeof(CS.UnityEngine.UI.ScrollRect))\r\n", trans.name, path);
        }
        else if (trans.name.EndsWith("Toggle"))
        {
            descSB.AppendFormat("\t\t---@field public {0} UnityEngine.UI.Toggle\r\n", trans.name);
            contentSB.AppendFormat("\t\tuis.{0}=ui:Find(\"{1}\"):GetComponent(typeof(CS.UnityEngine.UI.Toggle))\r\n", trans.name, path);
        }
        else if (trans.name.EndsWith("ToggleGroup"))
        {
            descSB.AppendFormat("\t\t---@field public {0} UnityEngine.UI.ToggleGroup\r\n", trans.name);
            contentSB.AppendFormat("\t\tuis.{0}=ui:Find(\"{1}\"):GetComponent(typeof(CS.UnityEngine.UI.ToggleGroup))\r\n", trans.name, path);
        }
        else if(trans.name.EndsWith("Dropdown"))
        {
            descSB.AppendFormat("\t\t---@field public {0} ZGame.UGUIExtention.Dropdown\r\n", trans.name);
            contentSB.AppendFormat("\t\tuis.{0}=ui:Find(\"{1}\"):GetComponent(typeof(CS.ZGame.UGUIExtention.Dropdown))\r\n", trans.name, path);
        }
    }
}