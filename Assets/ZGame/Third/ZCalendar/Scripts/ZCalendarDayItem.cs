...
namespace ZTools
{
    public class ZCalendarDayItem : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        public Button btn;
        public TextMeshProUGUI lunarTxt;
        [HideInInspector]
        ...
        public bool IsOn
        {
            set
            {
                if (isOn != value || isOn)
                {
                    ...
                    txt.color = isOn?SelectedColor:(isCanClick ? DefaultColor: DisableColor);   
                    if (value)
                    {
        ...
        void IsUnexpiredTime(DateTime time, DateTime crtTime)
        {
            ...
            if (compNum < 0)
            {
                ...
                isCanClick = false;
                txt.color = DisableColor;
                lunarTxt.color = DisableColor;
            }
        }
        /// <summary>
        /// 判断是否为本月日期
        ...
        {
            if (zCalendarController != null && zCalendarController.zCalendarModel != null) {
                if (!zCalendarController.zCalendarModel.isStaticCalendar)
                {
                    zCalendarController.zCalendar.ChoiceDayEvent -= ChangeState;
                }
