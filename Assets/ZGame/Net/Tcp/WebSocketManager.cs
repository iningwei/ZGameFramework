...
public class WebSocketManager : SingletonMonoBehaviour<WebSocketManager>
{
    ...
    public void Connect(string url, Dictionary<string, string> headerDic)
    {
        ...
        Debug.LogError("begin connect to ws");
    ...
    public void Send(UInt16 msgId, IMessage msg)
    {
        if (webSocket != null)
        {
            using (var ms = new MemoryStream())
            {
                string name = ProtobufMsgIDDesUtils.GetNameById(msgId);

                //DebugExt.Log($"send {name},{msgId} :{msg.ToString()}");
    ...
    void send(UInt16 msgId, byte[] data)
    {
        ...
        //Write msgId
        Int16 msgIdOrdered = useNetworkOrder ? IPAddress.HostToNetworkOrder((Int16)msgId) : (Int16)msgId;
