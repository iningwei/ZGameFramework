...
namespace ZGame
{
    public class RandomTool
    {
        ...
        public static int GenerateExceptionInt(int v1, int v2, int exceptionValue)
        {
            while (true)
            {
                int v = GenerateInt(v1, v2);
                if (v != exceptionValue)
                {
                    return v;
                }
        ...
        /// <returns></returns>
        public static float GenerateFloat(float v1, float v2, UInt16 precision = 0)
        {
            int L;
            if (precision == 0)
            {
            ...
            int ran = GenerateInt(v1New, v2New);
        ...
        public static bool Ratio(float r)
        {
            if (r < 0 || r > 1)
            {
                DebugExt.LogE("error, r should [0,1]");
                return false;
            }
            int v = 0;
            int b = 1;
            ...
            {
                DebugExt.LogE("r should at least>=0.0001");
            }

            int randomV = GenerateInt(0, b);
        ...
        public static string RandomStr(int len)
        {
            ...
            for (int i = 0; i < len; i++)
            {
                result += allChar[RandomTool.GenerateInt(0, allChar.Length)];
            }
            return result;
        }
    }
